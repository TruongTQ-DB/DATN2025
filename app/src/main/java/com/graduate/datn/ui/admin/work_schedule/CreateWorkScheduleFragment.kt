...
import kotlinx.android.synthetic.main.fragment_create_work_schedule.*
...
class CreateWorkScheduleFragment : BaseFragment() {
    private val viewModelWorkSchedule: CreateWorkScheduleViewModel by activityViewModels()
    ...
    override val layoutId: Int
        get() = R.layout.fragment_create_work_schedule
...
//                .orderBy("date")
                .get().addOnSuccessListener { documents ->
                    ...
                    if (documents.isEmpty) {
                        toast("Bác sĩ chưa có lịch làm việc")
                        viewModelWorkSchedule.alreadyTime = false
    ...
    private fun getTimeWordSchedule(startTime: String, endTime: String) {
        ...
        viewModelWorkSchedule.scheduleTimeValue = getTimeSchedule(minutesBetween.toInt())
        edt_time_schedule.setText(viewModelWorkSchedule.scheduleTimeValue?.stringValue)
        viewModelWorkSchedule.alreadyTime = true
    }
    ...
    private fun updateUI() {
        ...
        val decorator = EventDecorator(Color.RED, selectedDays)
    ...
    override fun initData() {
        viewModelWorkSchedule.dataStaff.observe(viewLifecycleOwner) {
            if (::picker.isInitialized) {
                picker.addValueObject(it, viewModelWorkSchedule.staffId)
                hideLoading()
            }
        }
    }
    ...
    override fun initListener() {
        ...
        calendar_view.setOnDateChangedListener { _, date, selected ->
            if (selectedDays.contains(date)) {
                ...
                if (!viewModelWorkSchedule.staffId.isNullOrEmpty()) {
        ...
        edt_time_schedule.onAvoidDoubleClick {
            if (viewModelWorkSchedule.alreadyTime == false) {
                ...
                lifecycleScope.launch {
                    ...
                    picker.addValueObject(listScheduleTime,
                        viewModelWorkSchedule.scheduleTimeValue?.id)
                }

                picker.onclickProvincesPicker = { data ->
                    if (data is ScheduleTime) {
                        edt_time_schedule.setText(data.stringValue)
                        viewModelWorkSchedule.scheduleTimeValue = data
                    }
                }
            } else {
                toast("Bác sĩ đã có khung giờ làm việc!")
            }
        }

        edt_staff.onAvoidDoubleClick {
            showLoading()
            picker = BottomSheetWheelPicker(requireContext(), isUser = true)
            getDateStaff()
            picker.show(childFragmentManager, "")
            picker.onclickProvincesPicker = { data ->
                if (data is User) {
                    edt_staff.setText(data.name)
                    if (viewModelWorkSchedule.staffId != data.id) {
                        viewModelWorkSchedule.staffId = data.id.toString()
                        viewModelWorkSchedule.avatar = data.avatar.toString()
                        viewModelWorkSchedule.name = data.name.toString()
                        selectedDays.clear()
                        dayOld.clear()
                        getDataDate()
                    }
        ...
        btn_create.onAvoidDoubleClick {
            ...
            if (!validateCreate()) {
                ...
                val data = selectedDays.let { splitTimeRangesInto30MinuteSlots(it) }
                    .mapNotNull { (date, listTime) ->
                        ...
                        DateToWorkRequest(viewModelWorkSchedule.staffId,
                            viewModelWorkSchedule.avatar,
                            viewModelWorkSchedule.name,
                            day,
                            time,
    ...
    private fun validateCreate(): Boolean {
        if (edt_staff.text.trim().isEmpty()) {
            ...
            for (timeRange in timeRanges) {
                ...
                while (startCalendar.before(endCalendar)) {
                    ...
                    calendar.add(Calendar.MINUTE,
                        viewModelWorkSchedule.scheduleTimeValue?.intValue ?: 30)
    ...
    private fun getDateStaff() {
        ...
        userCollection.whereEqualTo("account", Constant.TYPE_ACCOUNT_BARBER_NAME).orderBy("name")
            .get().addOnSuccessListener { documents ->
                ...
                viewModelWorkSchedule.dataStaff.value = dataList
            }.addOnFailureListener {
                hideLoading()
                toast(R.string.error_400)
            }
...
data class ScheduleTime(val id: String? = "", val intValue: Int, val stringValue: String)

data class DateWorkResponse(val date: String? = "", val idBarberName: String? = "")