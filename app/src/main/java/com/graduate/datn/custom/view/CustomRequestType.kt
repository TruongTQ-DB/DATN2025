...
import com.graduate.datn.adapter.recyclerview.RequestTypeAdapter
...
class CustomRequestType(context: Context, attrs: AttributeSet?) :
    CustomViewConstraintLayout(context, attrs) {

    private var requestTypeAdapter: RequestTypeAdapter? = null
    ...
    override fun initView() {
        requestTypeAdapter = RequestTypeAdapter(context)
    ...
    enum class RequestType(val title: String) {
        onLeave("Nghỉ phép"), unpaidLeave("Nghỉ không lương")
    }

    fun getRequestType() = tv_request_type.text.toString().trim()
