...
import com.graduate.datn.custom.view.CustomRequestType
...
class RequestTypeAdapter(context: Context): RecyclerViewAdapter(context, true) {
    override fun initNormalViewHolder(parent: ViewGroup): RecyclerView.ViewHolder? {
        return RequestTypeViewHolder(parent.inflate(R.layout.item_select_request))
    }

    override fun bindNormalViewHolder(holder: NormalViewHolder, position: Int) {
        val item = getItem(position, CustomRequestType.RequestType::class.java)
        item?.let {
    ...
    inner class RequestTypeViewHolder(val view: View): NormalViewHolder(view){
        init {
            view.onAvoidDoubleClick {
                val item = getItem(adapterPosition, CustomRequestType.RequestType::class.java)
    ...
    var onClickItem :(CustomRequestType.RequestType) -> Unit = {}
}